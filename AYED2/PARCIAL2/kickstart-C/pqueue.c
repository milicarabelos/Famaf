...
{   
    ...
    if (q->size==0)
    {   
        for (unsigned int i = 0; i < q->minPrio+1 ; i++)
        {
            b = q->array[i] == NULL;
        }
    }
        ...
        bool existe=false;
        for (unsigned int i = 0; i < q->minPrio+1 && !(existe) ; i++)
        {
...
pqueue pqueue_empty(priority_t min_priority)
{
    ...
    q->size = 0;
    q->minPrio = min_priority;
    q->array = calloc(min_priority + 1, sizeof(first));
    assert(q->array != NULL);
...
pqueue_elem pqueue_peek(pqueue q)
{
    ...
    for (unsigned int i = 0; i <= q->minPrio+1&& b; i++)
    {
        if (q->array[i] != NULL)
        {
            p = q->array[i]->elem;
