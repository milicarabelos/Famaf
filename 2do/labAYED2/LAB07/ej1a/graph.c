...
{   
    bool b = true; 
    for (unsigned int i = 0; i < g->max_vertexs && b; i++)
    {
        for (unsigned int j = 0; j < g->max_vertexs && b; j++)
        {   
            unsigned int index = g_index(i,j,g->max_vertexs);
            if (i == j){
                b = g->costs[index] == 0 || g->costs[index]== cost_inf();                
            }
            else
            {
                b = g->costs[index] > 0;
